...
resource "aws_api_gateway_method" "post_subscriptions" {
  # checkov:skip=CKV2_AWS_53:Lambda proxy handler validates email/topics and rate limits writes.
  rest_api_id      = aws_api_gateway_rest_api.api.id
...
resource "aws_api_gateway_integration" "post_subscriptions" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.subscriptions.id
  http_method             = aws_api_gateway_method.post_subscriptions.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${var.contact_lambda_integration_arn}/invocations"
}

resource "aws_api_gateway_resource" "subscription_confirm" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  parent_id   = aws_api_gateway_resource.subscriptions.id
  path_part   = "confirm"
}

resource "aws_api_gateway_method" "post_subscription_confirm" {
  # checkov:skip=CKV2_AWS_53:Lambda proxy handler validates and consumes a high-entropy token.
  rest_api_id      = aws_api_gateway_rest_api.api.id
  resource_id      = aws_api_gateway_resource.subscription_confirm.id
  http_method      = "POST"
  authorization    = "NONE"
  api_key_required = true
}

resource "aws_api_gateway_integration" "post_subscription_confirm" {
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.subscription_confirm.id
  http_method             = aws_api_gateway_method.post_subscription_confirm.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${var.contact_lambda_integration_arn}/invocations"
}
...
resource "aws_api_gateway_deployment" "api" {
  depends_on = [
    ...
    aws_api_gateway_integration.post_subscriptions,
    aws_api_gateway_integration.post_subscription_confirm,
